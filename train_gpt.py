...
class Hyperparameters:
    ...
    eval_stride = int(os.environ.get("EVAL_STRIDE", 0))        # 0 = standard eval
    eval_batch_seqs = int(os.environ.get("EVAL_BATCH_SEQS", 32))

    # Quantization-aware training and int6 export.
...
def eval_val_sliding(
    ...
    if not (0 < stride < seq_len):
        raise ValueError(f"EVAL_STRIDE must satisfy 0 < stride < TRAIN_SEQ_LEN, got {stride} and {seq_len}")
    if batch_seqs <= 0:
        raise ValueError(f"EVAL_BATCH_SEQS must be positive, got {batch_seqs}")

    total_tokens = val_tokens.numel() - 1
    first_end = min(seq_len, total_tokens)
    window_ends = list(range(first_end, total_tokens + 1, stride))
    if not window_ends or window_ends[-1] != total_tokens:
        window_ends.append(total_tokens)

    total_windows = len(window_ends)
    my_s = (total_windows * rank) // world_size
    my_e = (total_windows * (rank + 1)) // world_size
    my_window_ids = list(range(my_s, my_e))
    ...
    base_model.eval()
    with torch.inference_mode():
        for bi in range(0, len(my_window_ids), batch_seqs):
            batch_ids = my_window_ids[bi : bi + batch_seqs]
            bsz = len(batch_ids)
            ...
            for i, window_idx in enumerate(batch_ids):
                end = window_ends[window_idx]
                prev_end = 0 if window_idx == 0 else window_ends[window_idx - 1]
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], fp16_embed: bool = False, int6_layer_start: int = -1, int6_layer_end: int = -1):
    ...
    for name, tensor in state_dict.items():
        ...
        # fp16 embedding passthrough (before counting as float tensor)
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self._qat and self.training:
            ...
            q_raw = (w / scale).round()
            if self._qat_int6:
                w_q = (q_raw / 4).round().mul_(4).clamp_(-128, 124) * scale
            else:
                w_q = q_raw.clamp_(-127, 127) * scale
            w = w + (w_q - w).detach()
...
def main() -> None:
    ...
    int6_active = (0 <= args.int6_layer_start <= args.int6_layer_end < args.num_layers)
    quant_tag = "mixed_int6" if int6_active else "int8"
    quant_filename = f"final_model.{quant_tag}.ptz"
    if master_process:
